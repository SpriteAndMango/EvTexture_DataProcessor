...
if __name__ == "__main__":
    ...
    xs = torch.from_numpy(events_video[:, 0].astype(np.int32)).reshape(-1, 1)
    ys = torch.from_numpy(events_video[:, 1].astype(np.int32)).reshape(-1, 1)
    ts = torch.from_numpy(events_video[:, 2].astype(np.float32)).reshape(-1, 1)
    ps = torch.from_numpy(events_video[:, 3].astype(np.int32)).reshape(-1, 1)
    t_start = ts[0]
    t_end = ts[-1]
    bins = 10
    sensor_size = (420, 420)
    ...
    voxel_video = events_to_voxel_torch(xs, ys, ts, ps, bins, device=None, sensor_size=sensor_size)
    normed_voxel_video = voxel_normalization(voxel_video)
    print(normed_voxel_video.shape)
